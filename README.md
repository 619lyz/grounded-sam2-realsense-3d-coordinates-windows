# Grounded SAM 2 RealSense Windows Edition

This repository is a Windows-adapted version of the Grounded SAM 2 camera demo.
It keeps the GroundingDINO + SAM2 continuous-ID tracking pipeline and adds a
Windows-friendly entry point for RealSense/OpenCV cameras.

## Windows Version Notes

- Main entry point: `main.py`
- Tested with a `dinosam2` conda environment on Windows
- Uses Windows camera backends (`DirectShow` / `Media Foundation`) instead of Linux `V4L2`
- Supports Intel RealSense color + depth input through `pyrealsense2`
- Keeps object masks, bounding boxes, continuous IDs, and 3D coordinate display in the live GUI
- Adds VS Code launch configs for OpenCV camera and RealSense camera

## Features

- Text-prompted object detection with GroundingDINO
- SAM2 instance segmentation and video tracking
- Continuous object IDs across frames
- RealSense depth alignment and per-object XYZ coordinate estimation
- Temporal smoothing for more stable coordinates
- Tkinter live preview window with prompt updates
- OpenCV fallback camera mode for debugging on Windows

## Quick Start on Windows

Create and activate the environment, then install PyTorch CUDA and project dependencies:
...
Run with a normal OpenCV camera:

```powershell
python main.py --source opencv --camera 0
```

For slower GPUs, reduce camera load and run GroundingDINO less often:
